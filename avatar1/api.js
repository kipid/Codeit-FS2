...
export async function createAvatar(avatarData) {
  ...
  if (!res.ok) {
    const message0 = (avatarData.hairType ? `` : `Field 'hairType' is required`)
    const message1 = (["black", "brown", "blonde"].some(el => el === avatarData.hairColor) ? `` : `Value for 'hairColor' must be one of black,brown,blonde`);
    let message = "";
